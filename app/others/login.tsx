  ...
  const submitHandler = async () => {
    ...
    try {
      const response = await fetch(
        `${REACT_APP_API_URL}/api/Security/CreateToken`,
        {
          method: "POST",
          headers: {
      ...
      if (response.status === 401 || response.status === 500) {
        ...
        // Should be user.code in link

        const crewIdResponse = await fetch(
          `${REACT_APP_API_URL}/api/User/GetByCode?code=${user.code}`,
          {
            headers: {
              "Content-Type": "application/json",
        ...
        const { crewId } = await crewIdResponse.json();

        if (!crewId) {
          setError("You are not crew. Only crew can access the app");
        } else {
          const userData = {
            token: token,
            code: user.code,
          ...
          await context.setUser(userData);
          router.replace("/");
        }
      } else {
